...
rf_building = RandomForestClassifier(n_estimators=100, random_state=0)
rf_floor = RandomForestClassifier(n_estimators=100, random_state=0)
rf_position = RandomForestClassifier(n_estimators=100, random_state=0)

rf_building.fit(X_train, y_train_building)
rf_floor.fit(X_train, y_train_floor)
rf_position.fit(X_train, y_train_position)

# Test kümesi üzerinde tahmin yapma
y_pred_building = rf_building.predict(X_test)
y_pred_floor = rf_floor.predict(X_test)
y_pred_position = rf_position.predict(X_test)
...
print("Random Forest modelinin doğruluk skoru PCA'SIZ (BUILDINGID):", accuracy_building)
print("Random Forest modelinin doğruluk skoru PCA'SIZ (FLOOR):", accuracy_floor)
print("Random Forest modelinin doğruluk skoru PCA'SIZ (RELATIVEPOSITION):", accuracy_position)
...
# Random Forest modelini oluşturma ve eğitme
rf_building_PCA = RandomForestClassifier(n_estimators=100, random_state=0)
rf_floor_PCA = RandomForestClassifier(n_estimators=100, random_state=0)
rf_position_PCA = RandomForestClassifier(n_estimators=100, random_state=0)

rf_building_PCA.fit(X_train_PCA, y_train_building_PCA)
rf_floor_PCA.fit(X_train_PCA, y_train_floor_PCA)
rf_position_PCA.fit(X_train_PCA, y_train_position_PCA)

# Test kümesi üzerinde tahmin yapma
y_pred_building_PCA = rf_building_PCA.predict(X_test_PCA)
y_pred_floor_PCA = rf_floor_PCA.predict(X_test_PCA)
y_pred_position_PCA = rf_position_PCA.predict(X_test_PCA)
...
print("Random Forest modelinin doğruluk skoru PCA'LI (BUILDINGID):", accuracy_building_PCA)
print("Random Forest modelinin doğruluk skoru PCA'LI (FLOOR):", accuracy_floor_PCA)
print("Random Forest modelinin doğruluk skoru PCA'LI (RELATIVEPOSITION):", accuracy_position_PCA)
print("a")
